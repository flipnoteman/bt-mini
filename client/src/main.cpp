...
Element RenderSyncTable(AppState &state) {
  ...
  // This is to decorate the synced column to be red for false and green for
  // true!
...
int main(int argc, char **argv) {
  ...
  Component main_view = Renderer([&] {
    ...
    Element main_panel =
        hbox({vbox({vbox({text("BT-Mini Client") | bold, separator(),
                          text("Configured endpoint: " + std::string(sec) +
                               "://" + state.temp.host + ":" + state.temp.port +
                               state.temp.target),
  ...
  app = CatchEvent(app, [&](Event e) {
    ...
    if (!state.show_options && e == Event::Character('s')) {
      ...
      std::thread([&] {
        screen.Post([&] {
          state.torrent_entries = scan_root_for_torrents(state.cfg.root_fs);
          state.status = "Scanned root: " + state.cfg.root_fs + " (" +
